...
class DayPage extends React.Component {
  ...
  handleGenreChange(event) {
    ...
    const thisGenre = this.props.data.eventsByGenre.group.find(
      genre => genre.fieldValue === thisGenreId
    );
    ...
    if (value === true) {
      ...
      this.setState(previousState => {
        const otherGenres = previousState.showingGenres.filter(
          genre => !(genre === thisGenre)
        );
        return {
    ...
    return (
      <Layout>
        <MyContainer>
          ...
          <Foot>
            <OptionButton onClick={this.toggleOptions}>
              Filter by genre
            </OptionButton>
          ...
          {showFilters && (
            <OptionsWindow>
              <CloseBalloon onClick={this.toggleOptions}>X</CloseBalloon>
              <h3 style={{ marginBottom: "0.5em" }}>Filter by genre</h3>
              <CheckboxList>
                {genres.map(genre => {
                  let genreId = genre.fieldValue;
                  let isShowing = showingGenres
                    ? showingGenres.includes(genre)
                    : true;
                  return (
                    <CheckboxContainer
                      key={"checkbox-" + date + genreId}
                      ticked={isShowing}
