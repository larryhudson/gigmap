...
async function main2() {
  ...
  const newEvents = eventsFromWeb.filter(
    ({ slug }) => !existingEventSlugs.includes(slug)
  );
  ...
  scrapedEvents.forEach(newEvent => {
    ...
    var eventIndex = events.findIndex(event => {
      return event.slug === newEvent.slug;
    });
    events[eventIndex] = newEvent;
  });
  
  // log number of scraped events
  console.log(
    events.filter(e => e.scraped).length + " scraped events in the events array"
  );
  ...
  const makeEventsJSON = await makeJSON(
    combinedEvents,
    "../site/src/data/events.json"
  );
  const makeVenuesJSON = await makeJSON(
    combinedVenues,
